...
class Tiago (Supervisor):
    def __init__(self, enable_joystick, people_number):
        ...
        self.camera_comp_color_publisher = rospy.Publisher("/xtion/rgb/image_raw/compressed", CompressedImage, queue_size=5)
        self.camera_comp_depth_publisher = rospy.Publisher("/xtion/depth/image_raw/compressed", CompressedImage, queue_size=5)
    ...
    def get_camera_image(self, timestamp):
        ...
        color_cv = cv2.cvtColor(cv2.cvtColor(np.frombuffer(color, np.uint8).reshape(self.camera_color.getHeight(), self.camera_color.getWidth(), 4), cv2.COLOR_RGBA2RGB ), cv2.COLOR_BGR2RGB)
        # color_image = Image(data=color, height=self.camera_color.getHeight(), width=self.camera_color.getWidth(), encoding="rgb8")
        # color_image.header.stamp = timestamp
        self.camera_color_publisher.publish(self.cv_bridge.cv2_to_imgmsg(color_cv))
        # self.camera_comp_color_publisher.publish(self.cv_bridge.cv2_to_compressed_imgmsg(color_cv, dst_format = "png"))
        ...
        self.camera_depth_publisher.publish(self.cv_bridge.cv2_to_imgmsg(depth_cv))
        # self.camera_comp_depth_publisher.publish(self.cv_bridge.cv2_to_compressed_imgmsg(depth_cv, dst_format = "png"))
